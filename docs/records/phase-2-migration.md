...
| 决策 | 来源 | 结果 |
| --- | --- | --- |
| Loop / Runner 分离 | `docs/agent-loop.md` §3「保留」 | `src/myagent/agent/loop.py:120` / `src/myagent/agent/runner.py:87` |
...
| 会话 JSONL 追加式 + `last_archived` 预留 | `docs/memory.md` §5 | `src/myagent/session/manager.py:142`（Phase 3 后由 `src/myagent/session/base.py:51` 的契约描述） |
| Context 只做「原料」不做预算 | `docs/context.md` §4 | `src/myagent/agent/context.py:223`（Phase 3 已改为 section + 预算检查，裁剪留给 Phase 6） |
...
- `ContextBundle.transcript_start` 的语义改成「本轮新增消息的起点」
  （`src/myagent/agent/context.py:312`、`:322`），历史越长它越大；
